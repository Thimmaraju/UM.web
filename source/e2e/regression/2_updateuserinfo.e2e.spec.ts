...
describe('Update Hospital User Information', () => {
  let page: LoginPage;
  ...
  it('Update User Information', async () => {
    ...
    const userInfo = edituser.updateUserInfo();

    userlist.searchTxt().sendKeys('anilkumar.indresh');
    util.sleep(7000);

    const fName = userlist.searchedFirstName().getText();
    expect(fName).toContain(userInfo.FirstName);

    const lName = userlist.searchedLastName().getText();
    expect(lName).toContain(userInfo.LastName);
