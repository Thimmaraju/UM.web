...
import { OmniTokenRefreshService } from '../omni-token-refresh/omni-token-refresh.service';

@Injectable({
  providedIn: 'root'
})
export class OmniTokenValidationService {
  ...
  openDialog(): void {
    ...
    this._zone.run(() => {
      this._matDialog.open<LibRefreshDialogComponent>(LibRefreshDialogComponent, { width: '50vw', disableClose: true });
    });
  }
}
